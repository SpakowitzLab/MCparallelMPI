...
!    3         |  Circle of radius LBox/2 inside box of size LBox
      ...
      if (abs(DEL*NBINX-LBOX).gt.0.000001) then 
          print*, "DEL=", DEL
          print*, "NBINX=",NBINX
          print*, "LBOX=",LBOX
          ...
          Do ix=1,NBINX
              Do iy=1,NBINX
                  do iz=1,NBINX  
                      ...
                      do I=1,8
                          rsq=((corner(I,1)-LBox/2.0)**2+ & 
                              (corner(I,2)-LBox/2.0)**2+ &
                              (corner(I,3)-LBox/2.0)**2)
                          minr=min(minr,rsq)
                      ...
                      if (nc.eq.0) then
                          ! inside
                          V=DEL**3
                      elseif ((nc.eq.8).and.(minr.gt.Rsqrd+DEL*0.5)) then
                          V=0.0
                      else
                          V=0.0
                          do I=1,npts
                              call random_number(urand,rand_stat)
                              rsq=( (corner(1,1)-urand(1)*DEL-LBox/2.0)**2+ &
                                   (corner(1,2)-urand(2)*DEL-LBox/2.0)**2+ &
                                   (corner(1,3)-urand(3)*DEL-LBox/2.0)**2)
